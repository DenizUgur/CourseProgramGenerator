...
async function test(input, uhours = "", size = false) {
  if (world === undefined) {
    return getWorld().then(async (w) => {
      world = w;
      const tables = await getResult(world, input, uhours);
      if (size) {
...
describe("Algorithm Test", () => {
  it("shouldn't give anything when NAN.101 is given", async () => {
  ...
  it("should give 4 course when ENG.102 & CS.102 and unavailable hours '2,09:40 2,12:40' are given", () => {
    return expect(
      test("ENG.102 CS.102", "2,09:40 2,12:40", true)
    ).resolves.toBe(2);
  });
